...
bme280_status_t bme280_init(bme280_t* dev, const bme280_bus_t* bus) {
    if (!dev || !bus) { return BME280_STATUS_INVALID_ARG; }
    ...
    if (currentID != BME280_ID) { return BME280_STATUS_INVALID_ARG; }

    return BME280_STATUS_OK;
}

bme280_status_t bme280_configure(bme280_t* dev, const bme280_config_t* cfg) {
    if (!dev || !dev->bme280_bus.read || !dev->bme280_bus.write || !cfg) { return BME280_STATUS_INVALID_ARG; }
    bme280_bus_t* bus = &dev->bme280_bus;

    bme280_mode_t intendedMode = dev->bme280_mode;
    if (bme280_set_mode(dev, SLEEP_MODE) != BME280_STATUS_OK) { return BME280_STATUS_ERROR; }
    dev->bme280_mode = intendedMode;

    uint8_t configReg = (cfg->bme280_standby | cfg->bme280_filter | cfg->bme280_spi_status);
    if (bus->write(bus->dev_addr, BME280_CONFIG_ADDR, &configReg, 1, bus->ctx) != BME280_STATUS_OK) {
        return BME280_STATUS_ERROR;
    }
    configReg = (uint8_t)dev->bme280_humidity_ovs;
    if (bus->write(bus->dev_addr, BME280_CTRL_HUM_ADDR, &configReg, 1, bus->ctx) != BME280_STATUS_OK) {
        return BME280_STATUS_ERROR;
    }
    configReg = (uint8_t)((uint8_t)dev->bme280_temperature_ovs | (uint8_t)dev->bme280_pressure_ovs | (uint8_t)dev->bme280_mode);
...
bme280_status_t bme280_reset(bme280_t* dev) {
    if (!dev || !dev->bme280_bus.write) { return BME280_STATUS_INVALID_ARG; }
...
bme280_status_t bme280_set_mode(bme280_t* dev, bme280_mode_t mode) {
    if (!dev || !dev->bme280_bus.read || !dev->bme280_bus.write) { return BME280_STATUS_INVALID_ARG; }
...
bme280_status_t bme280_read(bme280_t* dev, bme280_int32_t* T, bme280_uint32_t* H, bme280_uint32_t* P) {
    ...
    *T = (bme280_int32_t)((t_fine * 5 + 128) >> 8) / 100;
...
bme280_status_t bme280_weatherMonitoring(bme280_t* dev, bme280_config_t* cfg) {
    if (!dev || !cfg) { return BME280_STATUS_ERROR; };
